...
font = pygame.font.Font(r"fonts\comicbd.ttf", 14)
font = pygame.font.SysFont(None, 24)

icon = pygame.image.load(r'assets\icon.ico')
pygame.display.set_icon(icon)

logo = pygame.image.load(r"assets\small.png")
cat = pygame.image.load(r"assets\cat.png")
xmas = pygame.image.load(r"assets\xmas.png")
...
is_xmas = False
if is_xmas:
    background.blit(xmas,(0,0))

background.blit(logo,(1455,31))
...
#sla_thermometer = Thermometer(1920,)
...
class GUI_Row:
    ...
    def update(self):
        ...
        if self.csv_row[1] != " Activity":# if not headings, change colour of row based on activity
...
def create_agents():
    ...
    nmcgrath = Agent(False, 'nmcgrath4@dxc.com','Nathan','McGrath')
    ...
    wreid = Agent(False, 'william.reid@dxc.com', 'Bill', 'Reid')
    jboost = Agent(False, 'joshua.boost@dxc.com', 'Boost', 'J')
    nmclean = Agent(False, 'nicholas.mclean2@dxc.com', 'Nicholas', 'Mclean')
    chamilton = Agent(False, 'christopher.hamilton@dxc.com', 'Chris', 'Hamilton')
    xbrett = Agent(False, 'alexander.brett@dxc.com', 'Xander', 'Brett')
    zli = Agent(False, 'zhipeng.li@dxc.com', 'Zhipeng', 'Li')
    hyoung = Agent(False, 'hugh.young@dxc.com', 'Hugh', 'Young')
    tsheppard = Agent(False, 'tanisha.sheppard2@dxc.com', 'Tanisha', 'Sheppard')
    sknevett = Agent(False, 'sebastian.knevett@dxc.com', 'Seb', 'Knevett')
    uvazeele = Agent(False, 'ubadha.vazeele@dxc.com', 'Vaz', '')
    msu = Agent(False, 'majuan.su2@dxc.com', 'Majuan', 'Su')
    mkumari = Agent(False, 'emmanosha.kumari@dxc.com', 'Manusha', 'Kumari')
    njackimowicz = Agent(False, 'nicholas.jakimowicz@dxc.com', 'Nick', 'Jakimowicz')
    ntafurt = Agent(False, 'nicolas.tafurtgarcia@dxc.com', 'Tafurt', 'Nicolas')
    nshen = Agent(False, 'ningyuan.shen2@dxc.com', 'Ningyuan', 'Shen')
    ...
    agents_heading2 = Agent(False, 'Agents', 'Agents', ' ')

    rsingh = Agent(False, 'robin.singh3@dxc.com', 'Robin', 'Singh')
    dbenson = Agent(False, 'danielscott.benson@dxc.com', 'Daniel', 'Benson')
    dmcdougall2 = Agent(False, 'david.mcdougall2@dxc.com', 'David', 'McDougall')
    hxia = Agent(False, 'haifeng.xia2@dxc.com', 'Haifeng', 'Xia')
    ...
    ehandoko = Agent(False, 'eko.handoko@dxc.com', 'Eko', 'Handoko')
    jbaker = Agent(False, 'james.baker2@dxc.com', 'James', 'Baker')
    vmansuri = Agent(False, 'vanib.mansuri@dxc.com', 'Vanib', 'Mansuri')
    cbennett = Agent(False, 'cohen.bennett@dxc.com', 'Cohen', 'Bennett')
    hliang = Agent(False, 'hong.liang@dxc.com', 'Hong', 'Liang')
    skhosa = Agent(False, 'satwant.khosa@dxc.com', 'Satwant', 'Khosa')
    adarby = Agent(False, 'alex.darby@dxc.com', 'Alex', 'Darby')
    ndownham = Agent(False, 'nick.downham@dxc.com', 'Nick', 'Downham')
    chuang = Agent(False, 'chris.huang@dxc.com', 'Chris', 'Huang')
    ksingh = Agent(False, 'karanbir.singh@dxc.com', 'Karanbir', 'Singh')
    tjelly = Agent(False, 'thomas.jelly@dxc.com', 'Thomas', 'Jelly')
    rwise = Agent(False, 'richard.wise@dxc.com', 'Richard', 'Wise')
...
while is_running:
    ...
    if animation_play:
        background.blit(cat,(1000,1080-(timer*10)))
    if animation_done:
        background.blit(cat,(1000,1080-(240)))
    ...
    if timer >= (tick_rate * refresh_time):
        ...
        if(agents_enabled):
            ...
            if len(agent_reader_copy) >= 2:
                ...
                print("No info in New_Agents - bad refresh")
                ...
                try:
                    with open(queue_csv_path, newline='') as f:
                        ...
                        for queue_row in queue_reader:
                            ...
                            if queue_row[0] == "Summary":
                                sla_split = queue_row[9].split("%")[0]
                                #todo Thermometer
                                print(int(float(sla_split)*10))
                except:
        ...
        if len(agent_reader_copy) > 2:
            ...
            for row in agent_reader_copy:# count all agents in given catagories
                if row[1] == " On contact":
                ...
                elif row[1] == " Meetings/Huddles":
                    stats_other.amount += 1
                elif row[1] == " Projects (Internal)":
                    stats_other.amount += 1
                elif row[1] == " Missed":
                    stats_available.amount += 1
                    ...
                    stats_on_break.amount += 1
                elif row[1] == " Comfort Break":
                    stats_on_break.amount += 1
                elif row[1] == " Offline Tasks":
                    stats_other.amount += 1
                elif row[1] == " System Down":
                    stats_other.amount += 1
                elif row[1] == " Offline":
        ...
        for stats in stats_list:
            #print("refreshing " + stats.description )
